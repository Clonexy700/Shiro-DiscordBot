    ...
    async def buttons_controller(self, guild, current, source, channel, context):
        ...
        while current:
            ...
            if control == 'thumbnail':
                await channel.send(embed=discord.Embed(color=discord.Colour.dark_purple()).set_image(url=source.thumbnail).set_footer(
                    text=f"Requested by {source.requester} | Video: {source.title}",
                    icon_url=source.requester.avatar_url), delete_after=10)

            if control == 'tutorial':
                await channel.send(
                    embed=discord.Embed(color=discord.Colour.dark_purple()).add_field(name="How to use Music Controller?",
    ...
    async def player_loop(self):
        ...
        while not self.bot.is_closed():
            ...
            try:
                ...
                continue
            embednps = discord.Embed(color=discord.Colour.dark_purple())
            ...
            embednps.add_field(name="Song duration:", value=f"**{datetime.timedelta(seconds=source.duration)}**",
                               inline=True)
...
class Music(commands.Cog):
    ...
    async def now_playing_(self, ctx):
        ...
        embednp = discord.Embed(color=discord.Colour.dark_purple())
        ...
        embednp.add_field(name="Song duration:", value=f"**{datetime.timedelta(seconds=vc.source.duration)}**",
                          inline=True)
    ...
    async def queue_info(self, ctx):
        ...
        embed = discord.Embed(title=f'Queue - Next {len(upcoming)}', description=fmt, color=discord.Colour.dark_purple())
    ...
    async def disconnect_(self, ctx, *, channel: discord.VoiceChannel = None):
        ...
        if vc:
            try:
                ...
                raise VoiceConnectionError(f'Disconnecting from channel: <{channel}> timed out.')

    @commands.command(name='stop', aliases=['stopplay', 'pause'])
    async def stop_(self, ctx):
        vc = ctx.voice_client
        vc.pause()

    @commands.command(name='resume', aliases=['resumeplay', 'continue'])
    async def resume_(self, ctx):
        vc = ctx.voice_client
        vc.resume()

    @commands.command(name='skip', aliases=['songskip', 's', 'ski', 'skipp'])
